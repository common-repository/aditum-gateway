    ...
    try {
        (() => {
            ...
            const intervalID = setInterval(async () => {
                const aditumObj = (window).AditumAntifraud;
                let clear = limit / period <= ++nTry;
                if (
                    typeof aditumObj !== "undefined" &&
                    typeof aditumObj.getVisitorId !== "undefined"
                ) {
                    const visitorID = await aditumObj.getVisitorId();
                    console.log("loadAditum");
                    console.log(visitorID);
                    document.getElementById('antifraud_token').setAttribute('value',visitorID);
                    clear = true;
                }
...
if (getAntiFraudType()=='aditum') {
    try {
        const pk = getAntiFraudId();
        // @ts-ignore
        window.fingerprintToken = pk;
        ((a, b, c, d, e, f, g) => {
            a['AdtObject'] = e;
            a[e] = a[e] || function () {
                (a[e].q = a[e].q || []).push(arguments)
            ...
            // @ts-ignore
            g.parentNode.insertBefore(f, g)
        })(window, document, 'script', 'https://datascience-dev.aditum.com.br/docs/antifraud-fingerprint.js?now=' + Date.now(), 'csdp');
        loadAditum();
    } catch (error) {
    }
}
